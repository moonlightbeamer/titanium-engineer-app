...
6. **Lessons-learned entry quality**: RAG retrieval quality degrades with vague entries. The design must specify: (a) schema validation enforced by the CLI tool on `lessons_learned` entry submission — all four fields (problem description, code pattern, root cause, resolution) are required and must meet a minimum length; (b) whether a human approval step is required before an entry is embedded and becomes retrievable; (c) how stale or invalidated entries are deprecated without deleting retrieval history. An entry that says "be careful with async code" is noise — the CLI must reject it.
